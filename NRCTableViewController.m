...
-(void)checkProviderID{
    UIAlertController * alert=   [UIAlertController
                                  alertControllerWithTitle:@"Centralized Administration"
                                  message:@"You have chosen to use centralized administration. Provider ID verification."
                                  preferredStyle:UIAlertControllerStyleAlert];
    UIAlertAction* ok = [UIAlertAction actionWithTitle:@"Enter your Provider ID" style:UIAlertActionStyleDefault
                                               handler:^(UIAlertAction * action){
                                                    
                                                   // if central administration is active and without error, scan self.providers for
                                                   // a provider ID that matched the one entered by the user
                                                   //if(self.centralAdminActivated && !self.errorType){
                                                       UITextField *textField = alert.textFields[0];
                                                       NSString *text = textField.text;
                                                       self.providerID = text;
                                                   /*
                                                       id item;
                                                       BOOL found = NO;
                                                       for(item in self.providers){
                                                           if([self.providerID isEqual:item]){
                                                               found = YES;
                                                           }
                                                      // }
                                                       // if provider ID does not match, we log ther error and send it to
                                                       // the error store for permanent logging.
                                                       if (found == NO){
                                                           NSLog(@"Provider ID not found");
                                                           
                                                           self.errorType= @"NRC001 - Provider Id Did Not Match";
                                                           ...
                                                           UIAlertController *badProviderID = [UIAlertController alertControllerWithTitle:@"Provider ID does not match." message:@"The Provider ID that you entered does not match any of the Provider IDs from your services Central Administration! If you do not want to use Central Administration, go to the Settings for this app and turn OFF the Central Administration switch. Then hit the Home button and delete this execution of the program." preferredStyle:UIAlertControllerStyleAlert];
                                                           
                                                           UIAlertAction *idError = [UIAlertAction actionWithTitle:@"Press any key to continue" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action){
                                                               // set switch to request provide ID next time
                                                               [self setCentralAdminWasOn:NO];
                                                               [self dismissViewControllerAnimated:YES completion:nil];
                                                           }];
                                                           ...
                                                           [badProviderID addAction:idError];
                                                           [self presentViewController:badProviderID animated:YES completion:nil];
                                                       }
                                                       
                                                       [self checkPassword];
                                                   }
                                               */
                                                   [[NSUserDefaults standardUserDefaults]setValue:self.providerID forKey:@"providerID"];
                                                   [[NSUserDefaults standardUserDefaults]synchronize];
                                               }];
    
    [alert addAction:ok];
    
    // if settings bundle had central administration turned on and we successfully got data from the website, ask for the Provider ID.
        //if(self.centralAdminActivated && !self.errorType)
       // {
            [alert addTextFieldWithConfigurationHandler:^(UITextField *textField) {
                textField.placeholder = @"Enter your Provider ID?";
...
-(void)getinAppPurchases{
    ...
    [NSURLSessionConfiguration defaultSessionConfiguration];
    self.session = [NSURLSession sessionWithConfiguration:config delegate:nil delegateQueue:nil];
    self.urlString =@"http://www.nelsoncapes.com/emstimerspro/in_app_purchases.txt";
    self.centralURL1 = self.urlString;
    NSURL *url = [NSURL URLWithString:self.centralURL1];
    NSURLRequest *request = [NSURLRequest requestWithURL:url];
    NSURLSessionDataTask *dataTask =
    [self.session dataTaskWithRequest:request
                    completionHandler:^(NSData *data, NSURLResponse *response, NSError *error){
                        ...
                        if(jsonObject){
                            ...
                            // get the JSON data, sort, and log.
                            self.productIDS = [jsonObject objectForKey:@"in_app_purchases"];
...
-(void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender{
    ...
    if([segue.identifier isEqualToString:@"passcodeViewController"]){
        ...
        destViewController.passwordWasEntered = self.passwordEntered;
